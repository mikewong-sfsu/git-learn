#! /usr/bin/perl
# Languages from the US Library of Congress
...
while( <DATA> ) {
	chomp;
	my ($code3a, $code3b, $code2, $english, $french) = split /\|/;
	$languages->{ $code2 } = $english if $code2;
}
print $json->canonical->pretty->encode( $languages );
